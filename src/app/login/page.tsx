...
export default function LoginPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (USE_SUPABASE) {
      ...
      // onAuthStateChange in AppContext will update the user state
  ...
  const handleGoogle = async () => {
    ...
    if (USE_SUPABASE) {
      ...
      // Redirects to Google — no need to setLoading(false)
      return;
    }
    ...
    <div className="min-h-screen bg-slate-50 flex flex-col items-center justify-center px-4 py-12">
      <div className="w-full max-w-sm">
        ...
        <div className="bg-white rounded-2xl border border-slate-200 shadow-sm p-8">
          <h1 className="text-2xl font-black text-slate-900 mb-1">Entrar</h1>
          <p className="text-slate-500 text-sm mb-6">Acesse sua conta no Mercadão MX</p>
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            ...
            <div>
              <div className="flex items-center justify-between mb-1.5">
                ...
                <button type="button" className="text-xs text-orange-500 hover:text-orange-600">Esqueci a senha</button>
              </div>
              <input
                type="password"
                required
